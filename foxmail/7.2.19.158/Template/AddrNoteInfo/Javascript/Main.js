...
window.D2J_UpdateMembers = function(members) {
	...
	if (count > 0) {$('.groupEdit_memberSection .errMemberMsg').hide();}
	InitEventHandler();
}
...
window.D2J_InputError = function(where) {
	...
	} else if (where == 'groupMember') {
		$('.groupEdit_memberSection .errMemberMsg').show();
	} else if (where = 'groupNameDup') {
		$('.groupName_edit .errNameDupMsg').show();
	}
}
