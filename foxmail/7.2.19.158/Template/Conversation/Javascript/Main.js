...
function getScrollHeight() {
	var totalHeight = 0;
	$('#body_main').children().each(function() {
		totalHeight += $(this).height();
	});
	return totalHeight - $('#body_main').height();
}

window.D2J_ScrollTo = function (x, y, width, height) {
	var scaleY = getScrollHeight()/height;
	var scaleX = $('.mail_content_panel').width()/width;
	window.scrollTo(x * scaleX, y * scaleY);
}

window.D2J_BindScroll = function(bind) {
	if (bind) {
		$(window).bind('scroll', function() {
			if (g_MouseIn) {
				g_ScrollTimer && clearTimeout(g_ScrollTimer);
				g_ScrollTimer = setTimeout(function() {
					J2D_OnScroll(this.scrollX, this.scrollY, $('.mail_content_panel').width(), getScrollHeight());
					g_ScrollTimer = null;
				}, 500);
			}
		});
...
window.D2J_ShowRecommendedBar = function() {
	try {
		var el = $('.mail_translate_info');
		el.slideDown(200);
	} catch (e) {
		window.J2D_WriteLog('JS:  D2J_ShowRecommendedBar: ' + e.message);
	}
}

window.D2J_GetMailTextForCorrectTransalte = function(callbackObj, srcLang, destLang, destText) {
	try {
		var textStr = '';
		if ($("[foxmail_class='mail_content_body']").length > 0) {
			textStr = $("[foxmail_class='mail_content_body']")[0].innerText;
		} else {
			var panelClone = $(".mail_content_panel").clone();
			panelClone.find('style').remove();
			panelClone.find('#mail_translate_footer').remove();
			textStr = panelClone.text();
		}
		return {
...
window.D2J_GetSelectionText = function(callbackObj, param) {
	try {
		var selection = window.getSelection();
		var textStr = selection.toString();
		...
		document.styleSheets[0].insertRule('::selection {background-color: #FFF75E;}', 0);
		return {
			obj_keys: 'param,textStr,offsetLeft,offsetTop,clientHeight,clientWidth',
			callbackObj: callbackObj,
			param: param,
			textStr: textStr,
			offsetLeft: left,
		...
		window.J2D_WriteLog('JS: D2J_GetSelectionText: ' + e.message);
		return {
			obj_keys: 'param,textStr,offsetLeft,offsetTop,clientHeight,clientWidth',
			callbackObj: callbackObj,
			param: param,
...
window.D2J_RemoveTranslateMark = function() {
	document.styleSheets[0].deleteRule(0);
}

window.D2J_ReplaceSubjectAndContent = function(subject, content, footer) {
	try {
		var bodyMainClone = $('#body_main').clone();
		bodyMainClone.find(".mail_content_panel").html(content);
		bodyMainClone.find('.mail_session_subject').html(subject);
		this.RemoveUnnecessaryPart(bodyMainClone);
		bodyMainClone.find('.mail_content_panel').append(footer);
		return {
			htmlStr: bodyMainClone[0].innerHTML
		};
	} catch (e) {
		window.J2D_WriteLog('JS: D2J_ReplaceSubjectAndContent: ' + e.message);
	}
}


window.D2J_GetCurrentMailHTML = function(callbackObj, param) {
	try {
		var content = $(".mail_content_panel").clone();
		content.find('blockquote').remove();
		var subjectStr = $('.mail_session_subject').html();
		var bodyMainClone = $('#body_main').clone();
		this.RemoveUnnecessaryPart(bodyMainClone);
		return {
			obj_keys: 'param,htmlStr,subject',
			callbackObj: callbackObj,
			param: param,
			htmlStr: content.html(),
			subject: subjectStr,
			srcHtml:  bodyMainClone.html()
		};
	} catch (e) {
		window.J2D_WriteLog('JS: D2J_GetCurrentMailHTML: ' + e.message);
	}
}
...
// 移除翻译文本中的多余html
//
function RemoveUnnecessaryPart(node) {
	node.find('.mail_preview_panel').remove();
	node.find('.mail_translate_info').remove();
	node.find('.fast_reply_panel').remove();
	node.find('.fastreply_button').remove();
	node.find('.control_button').remove();
	node.find('.mail_top_most').remove();
	node.find('.detail_info_prompt').remove();
}
...
function InitEventHandler() {
	try {
		...
		$('.mail_translate_icon_cross').unbind();
		...
		$('.mail_translate_link').unbind();
		$('.mail_info_corp_card_image').unbind();		
		...
		// 推荐翻译关闭
		//
		$('.mail_translate_icon_cross').click( function(){
			try {
				var el = $(this);
				var mailid = el.parents('.mail_session').attr('mailid');
				window.J2D_OnRecommandTranslateBarClosed(mailid);
				var el = $('.mail_translate_info');
				el.slideUp(200);
			} catch (e) {
				window.J2D_WriteLog('JS:  mail_translate_icon_cross click: ' + e.message);
			}
		});
		...
		// 立即翻译请求
		//
		$('.mail_translate_link').click( function(){
			try {
				window.J2D_TranslateNow();
			} catch (e) {
				window.J2D_WriteLog('JS:  mail_translate_link click: ' + e.message);
			}
		});
		...
		//企业卡片
		$('.mail_info_corp_card_image').mouseover(function () {
			var el = $(this);
			var mailid = el.parents('.mail_session').attr('mailid');
			var X = getLeftPos(el[0]);
			var Y = getTopPos(el[0]);			
			window.J2D_OnMouseOverCorpCardImage(mailid, X, Y, el[0].offsetWidth, el[0].offsetHeight);
		}).mouseout(function () {
			window.J2D_OnMouseOutCorpCardImage();
		})
